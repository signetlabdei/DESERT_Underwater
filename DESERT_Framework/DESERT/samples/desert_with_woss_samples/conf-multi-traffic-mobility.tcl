...
proc initRovMobility { } {
	global ns woss_utilities position_rov opt
  ...
  while { $toa < $opt(stoptime)} {
  	set curr_y [expr $curr_y0 + 90 * rand()]
  	set curr_x [expr $curr_x0 -100 + 200 * rand()]
  	...
  	set time_in_wp [expr $opt(rov_time_in_wp) + rand()*2*$opt(rov_time_in_wp)]
  	set toa      [$position_rov addWayPoint $curr_lat $curr_lon $curr_depth $opt(speed_rov) $time_in_wp]
    set time_in_wp [expr 100 * $opt(rov_time_in_wp) + rand()*2*$opt(rov_time_in_wp)]
...
proc initLeaderMobility { } {
	global ns woss_utilities position_leader opt
  ...
  set toa 0.0
  while { $toa < $opt(stoptime)} {
  	set curr_y [expr $curr_y0 - 50 * rand()]
  	set curr_lat    [ $woss_utilities getLatfromDistBearing  $opt(start_lat) $opt(start_long) 180.0 $curr_y ]
  	set time_in_wp [expr $opt(rov_time_in_wp) + rand()*2*$opt(rov_time_in_wp)]
  	...
  	set curr_y [expr $curr_y0 + 30 * rand()]
  	set curr_lat    [ $woss_utilities getLatfromDistBearing  $opt(start_lat) $opt(start_long) 180.0 $curr_y ]
  	set toa      [$position_leader addWayPoint $curr_lat $curr_lon $curr_depth $opt(speed_rov) $time_in_wp]
  	set toa      [$position_leader addWayPoint $curr_lat0 $curr_lon $curr_depth $opt(speed_rov) $time_in_wp]
  }
...
proc initDiverMobility { id } {
	global ns woss_utilities position_diver opt
  ...
  while { $toa < $opt(stoptime)} {
  	set curr_y [expr $curr_y0 + $max_movement_y * rand()]
  	set curr_x [expr $curr_x0 - $max_movement_x/2 + $max_movement_x * rand()]

  	set curr_lat    [ $woss_utilities getLatfromDistBearing  $opt(start_lat) $opt(start_long) 180.0 $curr_y ]
  	set curr_lon    [ $woss_utilities getLongfromDistBearing $opt(start_lat) $opt(start_long) 90.0  $curr_x ]
  	set time_in_wp [expr $opt(diver_time_in_wp) + rand()*2*$opt(diver_time_in_wp)]
