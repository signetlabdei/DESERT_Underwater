...
def compute_error(file_path,pkt_size):
        ...
        for row in file:
            ...
            if row != ['']:
                if float(row[0]) not in m_t:
                    ...
                    asv_e[float(row[0])] = [(rcv_s_er * (t/3600) + rcv_s*0.008/3600) * 0.8/1000 + ((sent_s_er + sent_s)*0.008/3600)*60/1000 + listen_time*0.01/1000]
                    ...
                    asv_e[float(row[0])].append((rcv_s_er * (t/3600) + rcv_s*0.008/3600) * 0.8/1000 + ((sent_s_er + sent_s)*0.008/3600)*60/1000 + listen_time*0.01/1000)
                    listen_time = 150000-(sent_u_er +rcv_u_er)*(t) -(sent_u +rcv_u)*(0.008)
    ...
    return m_t,m_d,m_tp,m_fp,m_tn,m_fn,acc,pr,rc, d_1, d_2, d_3, d_4, asv_e, auv_e


file_path = "result.csv"

m1_t,m1_d,m1_tp,m1_fp,m1_tn,m1_fn,acc1,pr1,rc1, d11,d12,d13,d14, asv_e, auv_e = compute_error(file_path,125)

file_path = "result_2.csv"

m2_t,m2_d,m2_tp,m2_fp,m2_tn,m2_fn,acc2,pr2,rc2,d21,d22,d23,d24,asv_e2, auv_e2 = compute_error(file_path,5)
